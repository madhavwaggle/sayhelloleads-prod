...
export function buildConversationPrompt({ agentName, lead, conversationHistory, calendlyUrl }) {
  ...
  // ── Detect showing/meeting confirmed ─────────────────────────────────────
  // If lead has agreed to a specific time, the conversation is complete.
  // AI should send a warm closing, NOT ask more qualifying questions.
  const showingConfirmed = (
    /\b(see you|sounds good|that works|works for me|confirmed|done deal|perfect|i'll be there|see you then|saturday|sunday|monday|tuesday|wednesday|thursday|friday)\b/i.test(allLeadText) &&
    /\b(10am|11am|12pm|1pm|2pm|3pm|4pm|5pm|6pm|7pm|8pm|9am|\d+(am|pm)|morning|afternoon|evening|weekend|saturday|sunday)\b/i.test(allLeadText + allAiText) &&
    /\b(showing|schedule|book|lock in|appointment|visit|see the|tour)\b/i.test(allAiText)
  );
  ...
  const nextBlock = nextToAsk.length > 0
    ...
    : showingConfirmed
    ? '\nCONVERSATION COMPLETE: A showing has been scheduled and confirmed. Send a warm, brief closing message — confirm the details, express excitement, and wish them well. Do NOT ask any more questions.'
  ...
  if (memoryLines.length > 0 && trimmedHistory.length >= 2) {
    ...
    if (lastUserIdx !== -1) {
      ...
      if (showingConfirmed) {
        memoryLines.push('[SHOWING CONFIRMED — conversation is complete — send a warm closing message only, NO more questions]');
      }
      finalMessages = [
